...
import { ArrowRight, BarChart3, Bot, CalendarDays, ChevronDown, CircleHelp, Eye, EyeOff, LayoutDashboard, ListChecks, Mail, MessageSquareText, Play, Settings, Sparkles, UsersRound, Workflow, Zap } from "lucide-react";
...
function Checklist({showWelcome,onWelcomeDone}:{showWelcome:boolean;onWelcomeDone:()=>void}){
  const [activeId,setActiveId]=useState("leads"); const [done,setDone]=useState<string[]>([]); const active=sections.find(s=>s.id===activeId)!; const total=sections.reduce((n,s)=>n+s.steps.length,0); const progress=Math.round(done.length/total*100);
  const counts=useMemo(()=>Object.fromEntries(sections.map(s=>[s.id,s.steps.filter(step=>done.includes(step.id)).length])),[done]);
  function finish(id:string){setDone(items=>items.includes(id)?items:[...items,id])}
  return <main className="app">{showWelcome&&<Welcome onContinue={onWelcomeDone}/>}<header className="topbar"><div className="topbrand"><Brand compact/></div><div className="announcement"><span><Zap size={16}/> Make lead follow-up effortless with SmartHub.</span><a href="#">Explore Pro</a></div><div className="top-actions"><span className="trial">14 days left</span><button aria-label="Help"><CircleHelp size={19}/></button><button className="avatar">J</button><span>James</span></div></header>
    ...
    <section className="page"><header className="welcome-bar"><div><h1>Welcome to SmartHub, James <span>👋</span></h1><p>Let’s get your workspace ready.</p></div><div><button className="btn subtle">Tour</button><button className="btn primary small">+ Create</button></div></header>
      <section className="launch-card"><div className="launch-head"><div><h2>Your SmartHub launch plan</h2><p>Choose an area and complete a few quick steps.</p></div><span className="progress-label">{progress}% · {done.length} of {total} complete</span></div><div className="progress-track"><i style={{width:`${progress}%`}}/></div>
        <div className="two-column"><section className="setup-panel"><div className="panel-heading"><div><h3>Recommended setup</h3><p>Start with the area most useful to you.</p></div></div><div className="accordion-list">{sections.map(section=>{const Icon=section.icon;const open=section.id===activeId;const count=counts[section.id];return <article key={section.id} className={`accordion ${open?"open":""}`}><button className="accordion-head" onClick={()=>setActiveId(section.id)}><span className={`section-icon ${section.tone}`}><Icon size={18}/></span><span className="accordion-title"><strong>{section.title}</strong>{open&&<small>{section.description}</small>}</span><span className="section-count">{count}/{section.steps.length}</span><ChevronDown size={17} className={open?"rotate":""}/></button>{open&&<div className="accordion-body"><div className="step-list">{section.steps.map(step=>{const completed=done.includes(step.id);return <div className={`step ${completed?"done":""}`} key={step.id}><div className="step-info"><strong>{step.title}</strong><span className="step-duration">{step.minutes} min</span></div><div className="step-actions">{completed?<button className="step-action completed" disabled><span aria-hidden="true">✓</span> Completed</button>:<><button className="step-skip" onClick={()=>finish(step.id)}>Skip</button><button className="step-action" onClick={()=>finish(step.id)}>Start</button></>}</div></div>})}</div></div>}</article>})}</div></section>
          <aside className="guide"><span className="eyebrow">Quick guidance</span><h3>Help for this area</h3><p className="guide-intro">A short guide for your current setup.</p><div className="video"><div><span>{active.title}</span><b>{active.duration}</b></div><h4>{active.guideTitle}</h4><p>{active.guideText}</p><button aria-label="Play guide"><Play size={21} fill="currentColor"/></button><div className="video-preview"><i/><i/><i/></div></div><div className="resources"><h4>Helpful resources</h4><button><span><Mail size={17}/></span><div><strong>{active.title} guide</strong><small>Simple steps and examples</small></div><ArrowRight size={16}/></button><button><span><ListChecks size={17}/></span><div><strong>Setup best practices</strong><small>Review a recommended setup</small></div><ArrowRight size={16}/></button></div></aside>
        </div>
      </section>
    </section>
  </main>
